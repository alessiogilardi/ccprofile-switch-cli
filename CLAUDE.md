...
- `Command-Use` reconciles env vars (`Set-EnvApiKey`/`Clear-EnvApiKey`, `Set-EnvBaseUrl`/`Clear-EnvBaseUrl`) unconditionally, even when the requested profile is already the active one — the `active` marker file reflects registry state, not this process's actual environment, which may carry a stale `ANTHROPIC_API_KEY` inherited from a parent process. Do NOT reintroduce an early `return` that skips env reconciliation on the "already active" path.
...
**Stale env vars in already-open terminals:** `Clear-EnvApiKey`/`Clear-EnvBaseUrl` (and their `Set-*` counterparts) only touch the current process's environment block plus the persisted `User`-level value in the registry. Running `ccprofile use <name>` in *this* terminal always reconciles *this* process's env vars against the target profile, even if the profile was already active (see invariant above) — but any *other* terminal pane that was already spawned before the switch (e.g. an IDE-integrated terminal) inherited its own copy of the environment at spawn time and will keep showing the old `ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` value until that terminal (or the IDE process spawning it) is restarted. That part is a Windows process-model limitation, not something fixable from within the script.
